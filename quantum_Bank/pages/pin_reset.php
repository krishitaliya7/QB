...
$otp_display = '';
if ($token) {
    $stmt = $conn->prepare('SELECT otp FROM pin_resets WHERE token = ?');
    $stmt->bind_param("s", $token);
    $stmt->execute();
    $result = $stmt->get_result();
    $reset = $result->fetch_assoc();
    $otp_display = $reset ? $reset['otp'] : 'Invalid token';
} else {
    $otp_display = 'No token';
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!verifyCsrfToken($csrf)) {
        ...
        $stmt = $conn->prepare('SELECT user_id, otp, expires_at FROM pin_resets WHERE token = ?');
        $stmt->bind_param("s", $token);
        $stmt->execute();
        $result = $stmt->get_result();
        $reset = $result->fetch_assoc();
        ...
        if ($reset && strtotime($reset['expires_at']) > time() && $reset['otp'] == $otp) {
            ...
            if ($stmt->execute()) {
                // Delete the token
                $stmt = $conn->prepare('DELETE FROM pin_resets WHERE token = ?');
                $stmt->bind_param("s", $token);
                $stmt->execute();

                audit_log($conn, 'pin.reset.success', $user_id, ['token' => $token]);
                $success = 'Your PIN has been reset successfully. You can now log in.';
            } else {
...
<head>
    ...
    <title>Reset PIN - QuantumBank</title>
...
<body class="min-h-screen bg-gray-50">
    <nav class="gradient-bg text-white shadow-lg sticky top-0 z-50" role="navigation" aria-label="Main navigation">
        ...
        <!-- OTP Box -->
        <div class="absolute top-16 right-4 bg-white text-gray-800 rounded-lg shadow-lg p-4 max-w-sm z-50" id="messagesBox">
            <h3 class="font-semibold mb-2">OTP</h3>
            <div id="messagesContent">
                <?php echo htmlspecialchars($otp_display); ?>
            </div>
            ...
            <div class="card p-8">
                ...
                <?php if (!$success): ?>
                    <form method="POST" id="resetForm" class="space-y-6">
                        ...
                        <div>
                            <label for="otp" class="block text-sm font-medium text-gray-700">OTP from Email</label>
                        ...
                        <div>
                            ...
                            <input type="password" id="pin" name="pin" placeholder="4-digit PIN" required maxlength="4" class="mt-1 block w-full border border-gray-300 rounded-lg py-2.5 px-4 text-sm input-focus">
                            <p class="error-message" id="pinError">PIN must be exactly 4 digits.</p>
                        </div>
                        <div>
                            <label for="confirm_pin" class="block text-sm font-medium text-gray-700">Confirm New PIN</label>
                            <input type="password" id="confirm_pin" name="confirm_pin" placeholder="Confirm 4-digit PIN" required maxlength="4" class="mt-1 block w-full border border-gray-300 rounded-lg py-2.5 px-4 text-sm input-focus">
                            <p class="error-message" id="confirmError">PINs do not match.</p>
                        </div>
                        <div>
                            <button type="submit" class="w-full py-2.5 px-4 btn-primary text-white rounded-lg font-medium focus:ring-2 focus:ring-accent focus:ring-offset-2">Reset PIN</button>
                        </div>
                    </form>
                <?php endif; ?>
